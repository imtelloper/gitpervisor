...
export function CropContextBar({
  ...
  return (
    <>
      ...
      <span className="shrink-0 text-fg-muted">크롭</span>
      ...
      <Sep />
      <span className="shrink-0 text-fg-dim">직선화</span>
      <input
        type="range"
        aria-label="직선화"
        title={limited ? `이미지가 커서 ±${maxDeg}°까지` : "직선화"}
        min={-maxDeg}
        max={maxDeg}
        step={0.1}
      ...
      {limited && (
        <span className="shrink-0 text-[11px] text-fg-dim">이미지가 커서 ±{maxDeg}°까지</span>
      )}

      <Sep />
      <Btn title="왼쪽 90°" onClick={() => api.cropTransform("rotCCW")}>
        <RotateCcw size={14} />
      </Btn>
      <Btn title="오른쪽 90°" onClick={() => api.cropTransform("rotCW")}>
        <RotateCw size={14} />
      </Btn>
      <Btn title="좌우 반전" onClick={() => api.cropTransform("flipH")}>
        <FlipHorizontal size={14} />
      </Btn>
      <Btn title="상하 반전" onClick={() => api.cropTransform("flipV")}>
        <FlipVertical size={14} />
      </Btn>

      <Sep />
      <Btn title="단색 여백을 찾아 크롭을 그 안쪽으로 맞춘다" onClick={api.cropAutoTrim}>
        <Scissors size={14} />
        여백 자동 제거
      </Btn>

      <div className="flex-1" />
      <Btn title="크롭 취소 (Esc)" onClick={api.cropCancel}>
        취소
      </Btn>
      <button
        type="button"
