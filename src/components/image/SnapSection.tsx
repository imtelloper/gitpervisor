...
function snapRowsFor(msg: Messages): {
  k: "snapPixel" | "snapObjects" | "snapGuides" | "smartGuides" | "gapBadges" | "rulers";
  label: string;
}[] {
  const t = msg.imageInspector.snap;
  return [
    { k: "snapPixel", label: t.snapPixel },
    { k: "snapObjects", label: t.snapObjects },
    { k: "snapGuides", label: t.snapGuides },
    { k: "smartGuides", label: t.smartGuides },
    { k: "gapBadges", label: t.gapBadges },
    { k: "rulers", label: t.rulers },
  ];
}

function gridStepsFor(msg: Messages): { v: Toggles["grid"]; label: string }[] {
  return [
...
export function SnapSection() {
  ...
  return (
    <section className="mt-2 first:mt-0">
      <div className="mb-1.5 text-[11px] text-fg-dim">{msg.imageInspector.snap.sectionTitle}</div>

      {snapRowsFor(msg).map((r) => (
        <label
          key={r.k}
      ...
      <div className="mt-2 flex items-center justify-between gap-2">
        <span className="text-[11px] text-fg-dim">{msg.imageInspector.snap.grid}</span>
        <select
          value={toggles.grid}
      ...
      <div className="mt-1.5 flex items-center justify-between gap-2">
        <span className="text-[11px] text-fg-dim">{msg.imageInspector.snap.threshold}</span>
        <input
          type="number"
          min={TOL_MIN}
          max={TOL_MAX}
