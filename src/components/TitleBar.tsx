...
export function FavoritesButton({ className }: { className?: string }) {
  ...
  const write = async (next: (cur: FavoriteFolder[]) => FavoriteFolder[]) => {
    try {
      ...
      useUi
        ...
        .pushToast(
          "error",
          msg.app.titleBar.favoritesSaveFailed(e instanceof Error ? e.message : String(e)),
        );
    }
  };
  ...
  const rename = (f: FavoriteFolder) =>
    useUi.getState().askPrompt({
      title: msg.app.titleBar.renameFavoriteTitle,
      label: f.path,
      defaultValue: f.name,
      confirmLabel: msg.app.titleBar.renameFavoriteConfirm,
      validate: (v) => (v.trim() ? null : msg.app.titleBar.renameFavoriteEmpty),
      onConfirm: (v) =>
        void write((cur) =>
  ...
  const browse = async () => {
    ...
    const picked = await pick({ directory: true, title: msg.app.titleBar.addFavoriteDialogTitle });
  ...
  return (
    <div className="relative mr-2.5" onClick={(e) => e.stopPropagation()}>
      <button
        onClick={() => setOpen((v) => !v)}
        title={msg.app.titleBar.favoritesButtonTitle}
        className={`flex items-center gap-1 rounded ${
          open ? "bg-raised text-accent" : "text-fg-muted hover:bg-raised hover:text-fg"
        } ${className ?? "px-1.5 py-0.5 text-[10px]"}`}
      >
        <FolderOpen size={11} /> {msg.app.titleBar.favoritesButton}
      </button>
        ...
        >
          ...
          {favs.length === 0 && unadded.length === 0 && (
            <div className="px-3 py-1.5 text-[11px] text-fg-dim">
              {msg.app.titleBar.favoritesEmpty}
            </div>
          )}
          ...
          {favs.map((f) => (
            ...
            >
              <button
                ...
                title={msg.app.titleBar.favoriteItemTitle(f.path)}
              ...
              <button
                ...
                title={msg.app.titleBar.favoriteRemoveTitle}
          ...
          {unadded.map((p) => (
            <button
              ...
              title={msg.app.titleBar.favoritePresetTitle(p.path)}
          ...
          >
            <Plus size={13} className="shrink-0" />
            <span>{msg.app.titleBar.favoriteAddFolder}</span>
          </button>
        </div>
      )}
    </div>
...
function AggregateButton() {
  ...
  return (
    <button
      ...
      title={
        windowOpen
          ? msg.app.titleBar.aggregateInWindowTitle
          : msg.app.titleBar.aggregateButtonTitle(hotkeyLabel)
      }
    ...
    >
      <LayoutGrid size={11} /> {msg.app.titleBar.aggregateButton}
    </button>
  );
}
  ...
  return (
    <>
      <button
        ...
        title={msg.app.titleBar.reportButtonTitle}
        className={`flex items-center gap-1 rounded ${
          reportOpen
            ? "bg-raised text-accent"
            : "text-fg-muted hover:bg-raised hover:text-fg"
      ...
      >
        <CalendarDays size={11} /> {msg.app.titleBar.reportButton}
      </button>
        ...
        >
          ...
          >
            ...
            >
              ...
              {msg.app.titleBar.reportOpenInWindow}
            </button>
          </div>
        </div>
      )}
  ...
  return (
    <>
      <button
        ref={btnRef}
        onClick={onClick}
        title={msg.app.titleBar.memoButtonTitle}
        className={`flex items-center gap-1 rounded ${
          anchor
            ? "bg-raised text-accent"
            : "text-fg-muted hover:bg-raised hover:text-fg"
      ...
      >
        <StickyNote size={11} /> {msg.app.titleBar.memoButton}
      </button>
      {anchor && (
        <GlobalMemoPopover
          anchor={anchor}
...
function QuarantineBadge() {
  ...
  return (
    <button
      ...
      title={msg.app.titleBar.quarantineTitle(count)}
    ...
    >
      <ShieldAlert size={12} />
      <span>{msg.app.titleBar.quarantineBadge(count)}</span>
    </button>
  );
}
