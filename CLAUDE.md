...
- **전체 e2e는 `node tests/e2e/shard.mjs`(앱 4개 병렬)로 3.5~5분이다**(순차 `run.mjs` 는 13~15분).
  앱·데이터·WebView2 폴더·픽스처를 샤드마다 따로 띄우므로 dev 앱이 안 떠 있어도 되고 설치본·dev 데이터도
  안 건드린다. 그래도 릴리스 전, 또는 공유 경로를 건드렸을 때만 돌린다. 그 외에는 `GPV_E2E_ONLY` 로
