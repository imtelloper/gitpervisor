...
기준은 세 가지다: **grep 한 번에 찾히는가 · 필요한 부분만 읽고 고칠 수 있는가 · 틀리면 컴파일러나
빠른 테스트가 잡는가.** 파일·함수 줄 수 제한은 없다.

**찾기**
- 이름은 저장소 전체에서 구체적이고 고유하게 짓는다(`handle`·`data`·`utils.ts` 금지). 같은 개념은
  Rust·TS·UI 문구·문서에서 같은 단어로 부른다.
- 식별자를 런타임에 조립하지 마라 — `` invoke(`${kind}_open`) ``, 동적 import 경로, 함수를 찍어내는
  매크로는 호출처를 검색에서 지운다. 재수출 전용 `index.ts`·import 별칭도 새로 만들지 않는다.

**IPC 경계 — 컴파일러가 못 보는 유일한 곳**
- 새 커맨드는 `#[tauri::command(async)]` + `Result<T, IpcError>`, TS 쪽은 `src/lib/ipc.ts`의 래퍼로 부른다.
  컴포넌트에서 `invoke`를 직접 부르지 않는다.
- **상태를 바꾸는 커맨드는 `callMutating`, 조회만 `call`.** `call`은 타임아웃(8초)이면 최대 3번까지 다시 호출하고
  같은 인자의 진행 중 호출을 합친다 — 변경 커맨드에 쓰면 두 번 실행된다.
- Rust 구조체 필드를 바꾸면 TS 타입도 **같은 커밋에서** 맞춘다. 어긋나도 양쪽 컴파일은 통과하고
  런타임에 `undefined`로만 드러난다. 인자 키는 Rust `project_id` ↔ TS `projectId`라 둘 다 검색한다.

**타입·실패**
- 새 코드에 `any` 금지, `as`는 DOM·외부 데이터 경계에서만. 상태는 문자열 말고 유니온/enum으로, 분기는
  `never` 검사로 빠짐없이.
- 에러를 삼키지 마라 — 빈 `catch`, `.ok()`로 버리기, `unwrap_or_default()`로 실패를 기본값에 숨기기.
  꼭 삼켜야 하면 이유를 한 줄 주석으로. 에러 메시지에는 대상(경로·id)을 넣는다.
- Rust 락은 `.unwrap_or_else(|e| e.into_inner())`, 뜨거운 커맨드는 `(async)` 유지 — `lib.rs`의
  `no_poison_propagating_unwraps`·`hot_commands_stay_async` 테스트가 잡는다.

**검증 가능하게**
- 계산·파싱·상태 전이는 UI·IPC와 떼어 순수 함수로 둔다. Rust는 같은 파일 `#[cfg(test)]` +
...
- 파일은 **따로 바뀌는 단위**로 나눈다. 2000줄이 넘으면 분할을 검토하고, 나눌 때는 로직 변경 없는
  이동만 담은 커밋으로 분리한다.
- 같은 블록을 세 번째 붙여 넣게 되면 함수로 뺀다(중복은 `Edit` 대상 특정도 막는다).
- 스타일은 주변 코드에 손으로 맞춘다. 포맷터를 파일 전체에 돌리지 않는다(아래 `cargo fmt` 참고).
- 구현 하나짜리 인터페이스·팩토리·계층, 쓰일 곳 없는 옵션·설정값을 만들지 않는다.
