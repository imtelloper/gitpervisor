...
export async function run({ cdp, report: r, fix }) {
  ...
  try {
    ...
    // 리마운트 직후엔 옛 에디터가 dispose 되고 새 것이 아직 없거나(-1), 새 에디터가 addAction 을 하기
    // 전(kb0 보다 작다)인 순간이 있다 — 둘 다 실측. 등록이 끝날 때까지 기다린다. 누수면 kb0 **초과**라
    // 이 대기는 곧바로 끝나고 아래 단언이 빨개진다.
    const kb1 = await poll(kbCount, (v) => v >= kb0, 40, 250);
