...
- 이동·삭제: 기존 가이드는 씬 캔버스 `[1]`이 포인터를 받으므로 `pointer.ts onPointerDown`(select 도구·`guidesVisible`)이 **핸들 히트 다음, 객체 히트 전**에 4 css px 거리 판정 → `DragState.guide{axis,index,orig}` + 커서 `col-resize/row-resize`(`updateHoverCursor`에 분기 1개). up: 이미지 밖이면 `'가이드 삭제'` 커밋, 안이면 `'가이드 이동'` 커밋(드래그 1회 = 1칸). 클릭(<3px)은 선택(`selectedGuide` — AnnotationLayer 로컬 ref, UI 상태) → 42 `delete` 액션이 `layerRef.deleteSelectedGuide()`를 먼저 부른다(반환 true면 소비). **"노드 선택이 비어 있을 때"라는 게이트는 쓸 수 없다** — 42 표의 `delete` 행이 `when:'hasSelection'`이라 노드 선택이 0이면 키 자체가 발화하지 않는다. 그 게이트를 넣으면 가이드를 Delete로 지울 방법이 영영 사라진다. 대신 **노드 선택이 바뀌면 `selectedGuide`를 비운다** — 그래야 둘 다 선택된 상태가 생기지 않아 무엇이 지워질지가 항상 분명하다(2026-09-09 구현 중 발견, 원 스펙이 실행 불가능).
