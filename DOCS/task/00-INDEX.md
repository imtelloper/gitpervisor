...
| # | 태스크 | 문서 | 규모 | 핵심 판단 | 주요 위험 |
|---|--------|------|------|-----------|-----------|
| 70 | 카탈로그 3종 → **5종**(`qwen3-4b-2507-q4`·`gemma4-e4b-qat-q4` 추가) | [70-llm-model-catalog-2026-09.md](70-llm-model-catalog-2026-09.md) | **S** | **Gemma 4는 Apache-2.0 + `gated:false`** — Gemma 3를 뺀 사유(실은 게이트가 아니라 **라이선스**)가 사라졌다. **llama-server는 jinja가 기본 ON**(`common.h:638`, SERVER 예제는 안 끈다)이라 Gemma 4가 인자 수정 없이 돌고 `enable_thinking:false`도 실제로 먹는다 — 레거시 내장 템플릿 54종 목록으로 후보를 떨어뜨리면 안 된다. **벤치마크 1위 2종이 실측에서 탈락**: Mi:dm 2.0 Mini는 KT 페르소나 템플릿이 영어 지시를 덮어 한→영 번역에 원문을 그대로 돌려주고(해시 인용 0), Gemma 4 12B는 12GB VRAM에서 생성 13 t/s(주간 요약 85초). `download_verified`가 Content-Length로 **재업로드를 먼저** 걸러 6.5GB 헛다운로드를 막는다. `-np 1` 추가(안 주면 슬롯 4개가 KV를 먹는다 — `chat.rs` 주석은 원래부터 `-np 1`이라고 적고 있었다) | 한국어 단독 벤치마크는 여전히 없다(여기 수치는 이 저장소 입력 4과제 한정) · 모델 대부분이 `## 날짜별` 머리글을 빠뜨린다(프롬프트 쪽 문제로 보임, 미수정) · 12B의 13 t/s 원인 미확정 |
