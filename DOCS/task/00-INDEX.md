...
## 14. 로컬 LLM 모델 카탈로그 확장 — Gemma 4 · Qwen3 2507 (70) — 2026-09-21

> 근거: HF API 실측(gated·lfs.oid·크기) + **앱과 같은 인자·같은 프롬프트로 7종 A/B**
> (llama-server b10809, `-c 8192 -ngl 99 -t 24`, `chat_template_kwargs:{enable_thinking:false}`)
> + 모델명을 감춘 **블라인드 채점**(과제 4 × 심사원 2).
> **문서 상태: 구현·검증 완료(2026-09-21, 미커밋)** — 상세 [70-llm-model-catalog-2026-09.md](70-llm-model-catalog-2026-09.md).
> 검증: `cargo test --lib catalog` ok · `tsc --noEmit` 0 · **dev 앱 실동작**(`llm_status` 5행 반환,
> Gemma 4로 `llm_chat` 7.5초 응답, 실행된 llama-server 명령줄에 `-np 1` 확인).

| # | 태스크 | 문서 | 규모 | 핵심 판단 | 주요 위험 |
|---|--------|------|------|-----------|-----------|
| 70 | 카탈로그 3종 → **6종**(`qwen3-4b-2507-q4`·`gemma4-e4b-qat-q4`·`gemma4-12b-qat-q4` 추가) + 요약 프롬프트 템플릿화 + 리포트 전용 모델 + 주간 예약 생성 | [70-llm-model-catalog-2026-09.md](70-llm-model-catalog-2026-09.md) | **S** | **Gemma 4는 Apache-2.0 + `gated:false`** — Gemma 3를 뺀 사유(실은 게이트가 아니라 **라이선스**)가 사라졌다. **llama-server는 jinja가 기본 ON**(`common.h:638`, SERVER 예제는 안 끈다)이라 Gemma 4가 인자 수정 없이 돌고 `enable_thinking:false`도 실제로 먹는다 — 레거시 내장 템플릿 54종 목록으로 후보를 떨어뜨리면 안 된다. **벤치마크 1위 2종이 실측에서 탈락**: Mi:dm 2.0 Mini는 KT 페르소나 템플릿이 영어 지시를 덮어 한→영 번역에 원문을 그대로 돌려주고(해시 인용 0), Gemma 4 12B는 12GB VRAM에서 생성 13 t/s(주간 요약 85초). `download_verified`가 Content-Length로 **재업로드를 먼저** 걸러 6.5GB 헛다운로드를 막는다. `-np 1` 추가(안 주면 슬롯 4개가 KV를 먹는다 — `chat.rs` 주석은 원래부터 `-np 1`이라고 적고 있었다). **요약 프롬프트를 산문 → 빈 템플릿 + `(100자 이내)`로 바꾸자 규격 외 머리글이 전 모델 0이 되고 2507의 잘림·8B의 불릿 위반이 사라졌다**(§8.1). 그 판에서 12B가 블라인드 14.11 vs 8B 9.22로 뒤집혀 카탈로그에 되돌렸다 — 8B는 실존 해시를 엉뚱한 커밋에 매다는 오귀속이 출력마다 4~6건이다(§8.2) | 한국어 단독 벤치마크는 여전히 없다(여기 수치는 이 저장소 입력 한정) · **예약은 프론트 타이머라 앱이 켜져 있어야 돈다**(§10 — 꺼져 있던 주는 다음 기동 때 따라잡는다) · 기능별 모델 전환마다 서버 재기동 9~15초(§9) · 기본 모델 4B는 프롬프트와 무관하게 주간 요약에 약하고 해시 인용이 회차마다 들쭉날쭉하다(§8.3, 결론 미정) · 8B는 길이 제한에서 근거 점수가 3.83→2.44로 떨어진다 · 12B의 13 t/s 원인 미확정 |

## 15. 영상 자동 자막 + 대본 기반 편집 (Vrew식) (72) — 2026-09-22

> 근거: 코드 실측 2026-09-22(515cd5f) + **엔진 실측**(whisper.cpp b5130 CPU/CUDA, FFmpeg whisper 필터, sherpa-onnx — 한국어 TTS) + Vrew·Descript 도움말.
> **문서 상태: 구현됨 P0~P4**(2026-09-22, P5 미착수) — 상세 [72-video-auto-subtitle-editor.md](72-video-auto-subtitle-editor.md). 설계안 3개 채점 후 합성(부록 A), 설계와 달라진 것·리뷰 후 수정은 §9. e2e 64·65(`E2E_NET`+`E2E_STT_MODEL`)·68.
