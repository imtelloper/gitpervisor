...
### ④가 공허했고, 그게 ⑤를 간헐 실패시키고 있었다 (2026-09-16)

⑤ `모든 경로 실패 → 사유 토스트`가 회차마다 다른 얼굴로 실패했다(`토스트=""` 또는 `클립=MARK`).
프로브를 심어 타임스탬프를 재니 원인이 하나였다:

```
copyText:in  id=2 (④ MARK, plugin 강제실패)   t=43605
fail []                                        t=43616   ← 스위트가 ⑤로 넘어감
copyText:in  id=3 (⑤ 센티널)                   t=43651
copyText:out id=3 ok=true                      t=43656   ← 센티널 기록
copyText:out id=2 ok=true                      t=43661   ← ④의 쓰기가 5ms 늦게 착지해 덮음
```

메뉴 [복사]는 `copyTerminalText` → `void copyText(...)` 라 **fire-and-forget** 이다. 각 단계는
`poll(clipboard, …MARK…)` 로 완료를 기다리는 척했지만, **③이 이미 MARK 를 넣어 둬서 ④의 폴은 첫
회차에 즉시 참이 됐다** — 아무것도 기다리지 않았고, ④의 쓰기가 ⑤의 기준선 위에 떨어졌다.
같은 공허함 때문에 **④는 폴백이 통째로 죽어도 초록이었다.**

수정: 단계마다 `primeClipboard(SENTINEL)` 로 기준선을 깐다(`__gpvClipboard.copy` 는 계층 쓰기를
**await 한다** — 메뉴 경로와 달리 끝난 시점이 확정된다). 그러면 폴이 진짜로 그 단계의 복사를
기다리고 경합이 사라진다. 기준선 성공 여부(`primed*`)도 단언에 넣어, 기준선이 실패한 회차가
"클립보드가 안 바뀌었다"로 조용히 통과하지 못하게 한다.

**공허하지 않음을 증명했다**: ④의 주입을 `["plugin","navigator","exec"]`(= 폴백 전멸)로 바꾸면
`클립="GPV-COPY-센티널"` 로 **빨개진다**. 수정 전에는 같은 상황에서 초록이었다.
