...
> ⚠ **태스크 36이 이 설계를 대체한다** — [36-project-color-32slot.md](36-project-color-32slot.md)
> (구현·검증 2026-09-03 · 이 주석 2026-09-04).
> 28이 세운 골격(색의 정의가 `lib/project-color.ts` 한 곳, 사이드바 행·모아보기 칩·셀 헤더가 같은 맵을 본다,
> 등록 전체 이름순 배정, 인라인 `--tint` + `bg-(--tint)`, "선택 행 기준선 이상"이라는 대비 목표 재정의)은
> **36이 그대로 승계한다.** 갈아엎은 것은 색을 만드는 방식이다: 12슬롯 → **32슬롯 + 이중 해싱**,
> 알파 틴트(`--proj-*` 5변수) → **불투명 OKLCH를 JS가 `#rrggbb`로 계산**, 테마별 명도 상수 → **토큰에서
> 이분탐색 유도**, 선택 표시 `border-l-2 border-accent` → **좌측 스트라이프 4→8px + accent outline**.
> 아래 §1~§9의 심볼(`projectTint`·`useProjectHues`·`assignProjectHues`·`TintLevel`·`--proj-a-row*`)과
> 알파 수치는 **더는 코드에 존재하지 않는다** — 그 시점의 설계·실측 기록으로만 읽어라. 본문은 보존한다.
>
