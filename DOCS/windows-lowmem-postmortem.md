...
#### 실측 결과 (2026-09-03, 디버그 앱 before/after)

기대는 private 100~150MB였는데 **실제는 26MB로 그보다 낮았다.** 버퍼·캐시 산술(366MB)만으로는
설명이 안 되는 몫이 더 있었다는 뜻이다 — 감시마다 딸린 HashMap 슬롯, `PathBuf` 키,
`roots` Vec 20,422 항목, 디바운서 내부 구조가 그것이다.

| | before (pid 32280) | after (pid 56148) | |
|---|---|---|---|
| gitpervisor.exe private | 448MB | **26MB** | −94.2% |
| 핸들 | 41,576 | **696** | −98.3% |
| 그중 File | ~20,500 | **48** | |
| 그중 Semaphore | ~20,470 | **63** | |
| 감시 수 | 20,422 | **22** | |
| 16KB 버퍼 | 319MB | **0.34MB** | |
| 스레드 | 171~223 | 130 | |

**File 핸들 48개가 가장 깨끗한 증거다** — 감시 디렉터리 핸들이 통째로 사라졌다. 세마포어 63은
그대로 감시 수로 읽으면 안 된다. 워처와 무관한 바닥이 ~41개 있어(before에서 세마포어 23,963 대
예측 감시 23,885의 차이 78이 그 몫이다) 빼면 22가 남고 등록된 루트 수와 정확히 맞는다.

등록 시간도 확인됐다. 22개 루트가 **한 초 안에** 전부 `감시 1개 (루트 재귀)`로 찍혔다.
구방식 마지막 시작은 9개 루트에 44초가 걸렸다(aipervisor 5,567 → germ 4,390 순으로 초 단위로
밀렸다). `[watcher]` 실패·경고·상한 로그는 0건이다.

수치가 "앱이 덜 떠서 낮은 것"이 아님도 확인했다: WebView2 트리가 완전히 올라와 있고
(browser 1, renderer 2, gpu 1, utility 2, crashpad 1), 등록된 22개 루트가 `projects.json`의
살아 있는 루트 수와 일치한다.

> 트리 전체(482MB)를 before(1,972MB)와 비교하면 안 된다. 그 차이의 큰 몫인 렌더러
> 1,206MB→219MB는 before 측정 때 e2e 부하가 걸려 있던 탓이다. 이번 수정의 순수한 성과는
> **gitpervisor.exe 본체의 448MB → 26MB**다.
